CREATE PROCEDURE [dbo].[sp_Roles_GetByPaging]
    @pageIndex INT = 1
    ,@pageSize INT = 10
...
BEGIN
    ...
    -- get total record
    SET @totalQuery = 'SELECT @totalRecordOut = COUNT(a.Id) FROM [dbo].[Roles] a WHERE (1=1) ';

    --get data by paging
    SET @pagingQuery = 'SELECT tmp.* FROM '+ 
                            '(SELECT RowIndex = ROW_NUMBER() OVER (ORDER BY ' + @sort + 
                            '), *  FROM [dbo].[Roles] a WHERE (1=1) ';
    ...
    IF ISNULL(@keyword,'') <> ''
    BEGIN
        DECLARE @keywordQuery NVARCHAR(MAX) = ' AND (a.[Name] LIKE N''%' + @keyword + '%'')';
